// /wwwroot/js/views/system-logs.js
...
/**
 * State management for system logs view
 */
const state = {
  currentPage: 1,
  pageSize: 10,
...
let systemClickHandler = null;

/**
 * Registers the system logs route
 * @param {Function} route - Route registration function
 * @returns {void}
 */
export function registerSystemLogs(route) {
  route('/logs/system', async (el) => {
    try {
...
async function loadLogs(el) {
  try {
    ...
    if (container) {
      container.innerHTML = `<div class="log-loading"><div class="spinner"></div><p>${t('admin.systemLogs.loadingLogs')}</p></div>`;
    }
    ...
    // Fetch logs
    /** @type {LogResponse} */
    const response = await api(`/log/system?${params.toString()}`);
...
function updateClearButton(el) {
  ...
  if (state.searchQuery) {
    ...
    if (!clearBtn) {
      ...
      clearBtn.innerHTML = `${icon(Icons.X, 'icon')} ${t('admin.systemLogs.clear')}`;
  ...
  return `
    <div class="log-table-container">
      <table class="log-table log-table--responsive">
        <thead>
          <tr>
            <th class="sortable ${state.sortBy === 'timestamp' ? 'sort-' + state.sortOrder : ''}" data-sort="timestamp" data-label="${t('admin.logs.timestamp')}">
              ${t('admin.logs.timestamp')}
            </th>
            <th class="sortable ${state.sortBy === 'user' ? 'sort-' + state.sortOrder : ''}" data-sort="user" data-label="${t('admin.logs.user')}">
              ${t('admin.logs.user')}
            </th>
            <th class="sortable ${state.sortBy === 'action' ? 'sort-' + state.sortOrder : ''}" data-sort="action" data-label="${t('admin.logs.action')}">
              ${t('admin.logs.action')}
            </th>
            <th class="sortable hide-mobile ${state.sortBy === 'context' ? 'sort-' + state.sortOrder : ''}" data-sort="context" data-label="${t('admin.logs.context')}">
              ${t('admin.logs.context')}
            </th>
            <th data-label="${t('admin.logs.message')}">${t('admin.logs.message')}</th>
          </tr>
        </thead>
        <tbody>
...
function renderLogRow(log) {
  return `
    <tr>
      ...
      <td data-label="${t('admin.logs.action')}" class="log-cell-action">${escapeHtml(log.action)}</td>
      <td data-label="${t('admin.logs.context')}" class="log-cell-context hide-mobile">${escapeHtml(log.context)}</td>
 ...
 * @param {Pagination} pagination - Pagination data
 * @returns {string} HTML string
 */
...
function setupEventHandlers(el) {
  if (systemClickHandler) {
    el.removeEventListener('click', systemClickHandler);
  }

  systemClickHandler = function handleClick(e) {
    if (!location.hash.startsWith('#/logs/system')) {
      return;
    }
  ...
  el.addEventListener('click', systemClickHandler);
